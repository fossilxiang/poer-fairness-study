...
\begin{itemize}
    \item \textbf{Blockchain consensus}: Proof-of-Work exhibits similar early-mover advantages\cite{nakamoto2008bitcoin,wang2025blockchain}
    \item \textbf{Participatory sensing}: Early data contributors receive disproportionate rewards\cite{ganti2011mobile,zhang2025entropy}
    \item \textbf{Carbon markets}: Vintage effects create intertemporal inequality\cite{worldbank2022carbon,rodriguez2025dynamic}
    \item \textbf{Data valuation}: Marginal contribution to model training diminishes with dataset size\cite{ghorbani2019data,li2025data}
    \item \textbf{Federated learning}: Recent work shows similar fairness challenges in incentive design\cite{chen2025federated,liu2025temporal}
    \item \textbf{Environmental governance}: Empirical studies confirm Matthew Effect in 50 cities\cite{thompson2025matthew}
\end{itemize}
...
Recent work on blockchain-based carbon markets\cite{tang2023blockchain,wang2025blockchain} emphasizes tokenization and MRV but lacks fairness analysis. F-PoER could integrate with such systems:
\begin{itemize}
    \item \textbf{On-chain enforcement:} Smart contracts implement F-PoER allocation rules with transparent, auditable execution\cite{liu2025temporal}
    ...
    \item \textbf{MRV integration:} IoT sensors provide verified $\mathbf{m}_i$ reports, reducing fraud risk\cite{mustafa2025tokenized}
    \item \textbf{Hybrid incentives:} Combining baseline guarantees with performance incentives aligns with recent findings\cite{garcia2025hybrid}
\end{itemize}
