...
\begin{tabular}{lccc}
\toprule
\textbf{Mechanism} & \textbf{Gini} & \textbf{Computation Time} & \textbf{Budget Balance} \\
\midrule
PoER (sequential) & 0.6234 & $O(n)$ & Yes \\
F-PoER & 0.1892 & $O(n)$ & No (surplus/deficit possible) \\
Shapley (exact) & 0.1523 & $O(2^n)$ & Yes \\
Shapley (sampling, $M=1000$) & 0.1687 & $O(M \cdot n)$ & Yes \\
\bottomrule
\end{tabular}
\end{table}
